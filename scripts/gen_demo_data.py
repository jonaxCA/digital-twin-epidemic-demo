...
""")
    ...
    demo_hash = gen_password_hash(demo_password)
    out.append("-- ---------------------------------------------------------------------------")
    out.append("-- Usuaria de demo para el recorrido de login. Password real (bcrypt):")
    out.append("--   usuario:   diana.flores")
    out.append(f"--   password:  {demo_password}   (documentada tambien en README.md)")
    out.append("-- SOLO para la demo local -- no usar este patron en un ambiente real.")
    out.append("-- ---------------------------------------------------------------------------")
    out.append(f"""INSERT INTO users (username, email, password_hash, full_name, is_active)
VALUES ('diana.flores', 'diana.flores@salud.nl.gob.mx', '{demo_hash}',
        'Diana Flores', TRUE)
